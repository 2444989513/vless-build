...
get_version() {
  ...
  TMP_FILE="$(mktemp)"
  if ! curl -x "${PROXY}" -sS -H "Accept: application/vnd.github.v3+json" -o "$TMP_FILE" 'https://api.github.com/repos/2444989513/vless-build/releases/latest'; then
    "rm" "$TMP_FILE"
...
download_v2ray() {
  DOWNLOAD_LINK="https://github.com/2444989513/vless-build/releases/download/$RELEASE_VERSION/v2ray-linux-$MACHINE.zip"
